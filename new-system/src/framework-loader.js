...
const PHASES_DIR = path.resolve(FRAMEWORK_DIR, 'phases');
const PROMPTS_DIR = path.resolve(FRAMEWORK_DIR, 'prompts');
...
async function loadCorePrinciples() {
  const filePath = path.join(FRAMEWORK_DIR, 'core-principles.yaml');
 ...
 */
async function loadPhaseFramework(phase) {
  ...
  const filePath = path.join(PHASES_DIR, `${phase}.yaml`);
  return loadYamlFile(filePath);
}
...
async function loadSystemPrompt(phase) {
  try {
    ...
    const promptsFile = path.join(PROMPTS_DIR, 'system-prompts.yaml');
    const prompts = await loadYamlFile(promptsFile);
 ...
 */
async function loadUserPromptTemplates(category, promptType) {
  try {
    // Load the user prompts file
    const promptsFile = path.join(PROMPTS_DIR, 'user-prompts.yaml');
    const prompts = await loadYamlFile(promptsFile);
 ...
 */
async function getPhaseFramework(phase) {
  try {
    const [corePrinciples, phaseFramework] = await Promise.all([
      loadCorePrinciples(),
      loadPhaseFramework(phase)
    ]);
 ...
 */
async function getFrameworkSection(phase, section) {
  try {
    const phaseFramework = await loadPhaseFramework(phase);
 ...
 */
async function createDynamicPrompt(phase, customizations = {}) {
  try {
    // Get base prompt for the phase
    let basePrompt = await loadSystemPrompt(phase);
